...
//AO CRIAR A BASE DE DADOS
  static Future<void> _onCreate(Database db, int version) async {
    await Funcoes_Centros.createCentrosTable(
        db); //CENTROS // TA A BUSCAR NA API
    ...
    await Funcoes_Topicos.createTopicosTable(
        db); // topicos das areas //TA A BUSCAR NA API

    await Funcoes_Topicos_imagens.createTopicosImagensTable(
        db); //IMAGENS DOS TOPICOS
    await Funcoes_Topicos_imagens.insertTopicosImagens(db);

///////////////////////////////
    await Funcoes_TipodeEvento.create_TipodeEVENTO_Table(
        db); //tipo de evento ->TA A BUSCAR DA API

    await Funcoes_Eventos.createEventoTable(db); //eventos->TA A BUSCAR DA API

    await Funcoes_Participantes_Evento.createParticipantesEventoTable(
        db); //lista de participantes evento->TA A BUSCAR DA API

    await Funcoes_Comentarios_Eventos.criarTabela_Eventos_Comentarios(
        db); //comentarios eventos->TA A BUSCAR DA API

    await Funcoes_Eventos_Imagens.criarTabela_Eventos_imagens(
        db); //galeria do evento->TA A BUSCAR DA API

//////////////////////////////////////////////////

    await Funcoes_Publicacoes.criarTabela_Publicacoes(
        db); //publicaçoes de locais ->TA A BUSCAR A API

    await Funcoes_Comentarios_Publicacoes.criarTabela_Publicacoes_comentarios(
        db); //comentarios dos locais ->TA A BUSCAR A API

    await Funcoes_Publicacoes_Imagens.criarTabela_Publicacoes_imagens(
        db); //imagens das publicacoes de locais //->TÁ A BUACAR A API

    await Funcoes_Publicacoes_Horario.criarTabela_Publicacoes_horario(
        db); //tabela do horario publicacao ->TA A BUSCAR NA API

    await Funcoes_AreasFavoritas.createAreasFavoritas_douserTable(
        db); //TA A BUSCAR NA API

    await Funcoes_TopicosFavoritos.createTopicosFavoritosUserTable(
        db); // TA A BUSCAR NA API

    await Funcoes_Partilhas.createPartilhasTable(
        db); //partilhas TA A BUSCAR DA API
    // await Funcoes_Partilhas.insertPartilhas(db);

    await Funcoes_Likes_das_Partilhas.createPartilhasLikesTable(
        db); //likes das partilhas fotos
    //await Funcoes_Likes_das_Partilhas.insertLikesPartilhas(db);

    await Funcoes_Comentarios_das_Partilhas.createPartilhasComentariosTable(
        db); //conentarios das partilhas fotos TA A BUSCAR A API
    ...
    await Funcoes_User_menbro_grupos.createUsuariomenbrodegruposTable(
        db); //user_menbro_de_grupos
    await Funcoes_User_menbro_grupos.insertUsuariomenbrodegrupos(db);

    await Funcoes_Mensagens_Grupos.createMensagemGrupoTable(
