...
import 'package:ficha3/PAGINA_INICIAL/PAGINA_PERFIL/pagina_eventos_do_user/pagina_guardar_altera%C3%A7%C3%B5es.dart';
...
class Pag_EditarEvento extends StatefulWidget {
  ...
  @override
  _Pag_EditarEventoState createState() => _Pag_EditarEventoState();
}

class _Pag_EditarEventoState extends State<Pag_EditarEvento> {
  ...
  String? local_do_evento_padrao;
  double? _latitude;
  double? _longitude;
  List<File> _imagensSelecionadas = [];

// Variáveis para armazenar os valores iniciais
  String? _initialTitulo;
  String? _initialDescricao;
  String? _initialLocal;
  DateTime? _initialDataInicio;
  DateTime? _initialDataFim;
  TimeOfDay? _initialHoraInicio;
  TimeOfDay? _initialHoraFim;
  int? _initialIdTipoEvento;
  int? _initialIdTopico;
  File? _initialImagemSelecionada;
  List<File> _initialImagensSelecionadas = [];
  ...
  Future<void> _carregarDadosEvento() async {
    List<Map<String, dynamic>> detalhesEventoList =
        await Funcoes_Eventos.consultaDetalhesEventoPorId(widget.id_evento);

    if (detalhesEventoList.isNotEmpty) {
      final detalhesEvento = detalhesEventoList.first;

      // Carregando o tipo de evento
      int? tipoEventoId = detalhesEvento['tipodeevento_id'];
      String? nomeTipoEvento;
      String? imagemTipoEvento;

      if (tipoEventoId != null) {
        Map<String, String> dadosTipoEvento =
            await Funcoes_TipodeEvento.obterDadosTipoEvento(tipoEventoId);

        nomeTipoEvento = dadosTipoEvento["nome"];
        imagemTipoEvento = dadosTipoEvento["imagem"];
      }

      // Carregando o tópico do evento
      int? topicoId = detalhesEvento['topico_id'];
      String? nomeTopico;
      String? imagemTopico;

      if (topicoId != null) {
        Map<String, String> dadosTopico =
            await Funcoes_Topicos.obterDadosTopico(topicoId);

        nomeTopico = dadosTopico["nome"];
        imagemTopico = dadosTopico["imagem"];
      }
      // Carregando imagens da galeria do evento
      List<String> caminhosImagens = await Funcoes_Eventos_Imagens()
          .consultaCaminhosImagensPorEvento(widget.id_evento);
      List<File> imagensSelecionadas =
          caminhosImagens.map((path) => File(path)).toList();

      // Extraindo e preparando os dados necessários
      final String titulo = detalhesEvento['nome'] ?? '';
      final String descricao = detalhesEvento['descricao_evento'] ?? '';
      ...
      final TimeOfDay horaFim = TimeOfDay(
        hour: int.parse(detalhesEvento['horas_acaba'].split(':')[0]),
        minute: int.parse(detalhesEvento['horas_acaba'].split(':')[1]),
      );

      final File? imagemSelecionada = detalhesEvento['caminho_imagem'] != null
          ? File(detalhesEvento['caminho_imagem'])
          : null;

      final double? latitude = detalhesEvento['latitude'];
      final double? longitude = detalhesEvento['longitude'];

      // Obtenha o endereço do local do evento com base nas coordenadas
      String? localDoEventoPadrao;
      if (latitude != null && longitude != null) {
        localDoEventoPadrao = await _carregarDetalhesLocal(latitude, longitude);
      }

      // Certifique-se de que `local_do_evento_padrao` está corretamente atribuído

      local_do_evento_padrao = localDoEventoPadrao;

// Armazenar valores iniciais
      _initialTitulo = titulo;
      _initialDescricao = descricao;
      _initialLocal = local_do_evento_padrao;
      _initialDataInicio = dataInicio;
      _initialDataFim = dataFim;
      _initialHoraInicio = horaInicio;
      _initialHoraFim = horaFim;
      _initialIdTipoEvento = tipoEventoId;
      _initialIdTopico = topicoId;
      _initialImagemSelecionada = imagemSelecionada;
      _initialImagensSelecionadas = List.from(imagensSelecionadas);
      ...
      setState(() {
        ...
        // Definindo o endereço padrão do evento
        local_do_evento_padrao = localDoEventoPadrao ?? 'errrrro';
        local_controller.text = local_do_evento_padrao!;

        _idTipoEventoSelecionado = tipoEventoId;
        _nomeTipoEventoSelecionado = nomeTipoEvento;
        _imagemTipoEventoSelecionado = imagemTipoEvento;

        _idTopicoSelecionado = topicoId;
        _nomeTopicoSelecionado = nomeTopico;
        _imagemTopicoSelecionado = imagemTopico;

        _dataSelecionada_inicio = dataInicio;
        _dataSelecionada_fim = dataFim;

        _horaSelecionada_inicio = horaInicio;
        _horaSelecionada_fim = horaFim;

        _imagemSelecionada = imagemSelecionada;
        _imagensSelecionadas = imagensSelecionadas;
      ...
      print(
          'Nenhum detalhe encontrado para o evento com id: ${widget.id_evento}');
    }
  }
  ...
  Future<String?> _carregarDetalhesLocal(
  ...
  bool _verificarModificacoes() {
    bool tituloModificado = _tituloController.text != _initialTitulo;
    bool descricaoModificada = _descricaoController.text != _initialDescricao;
    bool localModificado = local_controller.text != _initialLocal;
    bool dataInicioModificada = _dataSelecionada_inicio != _initialDataInicio;
    bool dataFimModificada = _dataSelecionada_fim != _initialDataFim;
    bool horaInicioModificada = _horaSelecionada_inicio != _initialHoraInicio;
    bool horaFimModificada = _horaSelecionada_fim != _initialHoraFim;
    bool tipoEventoModificado =
        _idTipoEventoSelecionado != _initialIdTipoEvento;
    bool topicoModificado = _idTopicoSelecionado != _initialIdTopico;
    bool imagemCapaModificada =
        _imagemSelecionada?.path != _initialImagemSelecionada?.path;
    bool imagensGaleriaModificadas = !_listasDeImagensSaoIguais(
        _imagensSelecionadas, _initialImagensSelecionadas);

    bool imagensSelecionadasVazias = _imagensSelecionadas.isEmpty;
    // Print para cada modificação com valores iniciais e atuais
    print(
        'Título modificado: $tituloModificado (Inicial: $_initialTitulo, Atual: ${_tituloController.text})');
    print(
        'Descrição modificada: $descricaoModificada (Inicial: $_initialDescricao, Atual: ${_descricaoController.text})');
    print(
        'Local modificado: $localModificado (Inicial: $_initialLocal, Atual: ${local_controller.text})');
    print(
        'Data de início modificada: $dataInicioModificada (Inicial: $_initialDataInicio, Atual: $_dataSelecionada_inicio)');
    print(
        'Data de fim modificada: $dataFimModificada (Inicial: $_initialDataFim, Atual: $_dataSelecionada_fim)');
    print(
        'Hora de início modificada: $horaInicioModificada (Inicial: $_initialHoraInicio, Atual: $_horaSelecionada_inicio)');
    print(
        'Hora de fim modificada: $horaFimModificada (Inicial: $_initialHoraFim, Atual: $_horaSelecionada_fim)');
    print(
        'Tipo de evento modificado: $tipoEventoModificado (Inicial: $_initialIdTipoEvento, Atual: $_idTipoEventoSelecionado)');
    print(
        'Tópico modificado: $topicoModificado (Inicial: $_initialIdTopico, Atual: $_idTopicoSelecionado)');
    print(
        'Imagem da capa modificada: $imagemCapaModificada (Inicial: ${_initialImagemSelecionada?.path}, Atual: ${_imagemSelecionada?.path})');
    print(
        'Imagens da galeria modificadas: $imagensGaleriaModificadas (Inicial: ${_initialImagensSelecionadas.map((file) => file.path).toList()}, Atual: ${_imagensSelecionadas.map((file) => file.path).toList()})');
    print('Imagens selecionadas vazias: $imagensSelecionadasVazias');
    if (imagensSelecionadasVazias) {
      return false; // Desativa se não houver imagens selecionadas
    }
    if (tituloModificado ||
        descricaoModificada ||
        localModificado ||
        dataInicioModificada ||
        dataFimModificada ||
        horaInicioModificada ||
        horaFimModificada ||
        tipoEventoModificado ||
        topicoModificado ||
        imagemCapaModificada ||
        imagensGaleriaModificadas) {
      return true; // Alguma informação foi modificada
    }
    return false; // Nenhuma modificação foi feita
  }

  bool _listasDeImagensSaoIguais(List<File> lista1, List<File> lista2) {
    if (lista1.length != lista2.length) return false;

    for (int i = 0; i < lista1.length; i++) {
      if (lista1[i].path != lista2[i].path) {
        return false;
      }
    }
    return true;
  ...
  void _updateButtonState() {
    final bool hasChanges = _verificarModificacoes();

    isButtonEnabled.value = hasChanges;

    // Adicionando o print para mostrar se o botão está ativo ou inativo
    if (isButtonEnabled.value) {
      print('Botão Guardar Alterações: Ativo');
    } else {
      print('Botão Guardar Alterações: Inativo');
    }
  }
  ...
  Future<bool> _verificarConectividade() async {
    ...
    if (connectivityResult == ConnectivityResult.none) {
      ...
      try {
        ...
        return false; // Falha na verificação adicional
      }
    }

    return false; // Se a verificação adicional falhar
  }

  void _navegarParaAtualizacao(BuildContext context) async {
    bool conectado = await _verificarConectividade();
    print('Conectado à internet: $conectado');
    if (conectado) {
      // Se houver conexão, navega para a próxima página
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => EventoUpdaterWidget(
            eventoId: widget.id_evento,
            idTopico: _idTopicoSelecionado!,
            idArea: widget.idArea,
            dataInicio: _dataSelecionada_inicio,
            dataFim: _dataSelecionada_fim,
            horaInicio: _horaSelecionada_inicio,
            horaFim: _horaSelecionada_fim,
            local: local_controller.text,
            idTipoEvento: _idTipoEventoSelecionado,
            titulo: _tituloController.text,
            descricao: _descricaoController.text,
            capa: _imagemSelecionada,
            imagensGaleria: _imagensSelecionadas,
          ),
        ),
      );
    } else {
      // Se não houver conexão, exibe um Scaffold com uma mensagem de erro
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Row(
            children: const [
              Icon(Icons.wifi_off, color: Color.fromARGB(255, 255, 255, 255)),
              SizedBox(width: 8),
  ...
  Future<bool> mostrarDialogoConfirmacao(BuildContext context) async {
    return await showDialog<bool>(
          ...
          builder: (context) {
            return AlertDialog(
              ...
              content: const Text(
                'Deseja guardar as alterações feitas no Evento?',
                textAlign: TextAlign.center,
              ),
              actions: [
                TextButton(
                            ...
                            TextField(
                              ...
                              decoration: const InputDecoration(
                                ...
                                focusedBorder: OutlineInputBorder(
                                  borderSide:
                                      BorderSide(color: Color(0xFF15659F)),
                                ),
                              ),
                            ),
                            ...
                            children: [
                              TextField(
                                ...
                                decoration: const InputDecoration(
                                  ...
                                  hintStyle:
                                      TextStyle(color: Color(0xFF6C757D)),
                                  border: OutlineInputBorder(
                                    borderSide: BorderSide(
                                        color:
                                            Color.fromARGB(150, 158, 158, 158)),
                        ...
                        Padding(
                          ...
                          child: OutlinedButton(
                            ...
                            style: OutlinedButton.styleFrom(
                              ...
                              side: const BorderSide(color: Color(0xFF15659F)),
                              shape: RoundedRectangleBorder(
                            ...
                            builder: (context, isEnabled, child) {
                              return ElevatedButton.icon(
                                onPressed: isEnabled
                                    ? () async {
                                        bool shouldSave =
                                            await mostrarDialogoConfirmacao(
                                                context);

                                        if (shouldSave) {
                                          _navegarParaAtualizacao(context);
                                        }
                                      }
                                    : null,
                                icon: const Icon(
